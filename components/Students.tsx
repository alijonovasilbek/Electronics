...
interface StudentsProps {
  students: Student[];
  groups: Group[];
  onAddStudent: (studentData: { name: string; dob: string; groupId: number; status: StudentStatus; }) => void;
}
...
const Students: React.FC<StudentsProps> = ({ students, groups, onAddStudent }) => {
    ...
    const handleAddStudent = (newStudentData: { name: string; dob: string; groupId: number; status: StudentStatus; }) => {
                    ...
                    <table className="w-full text-left">
                        <thead className="bg-gray-50">
                            <tr className="text-sm text-text-secondary">
                                ...
                                <th className="py-3 px-4 font-medium">Contact Phone</th>
                        ...
                        <tbody>
                            {filteredStudents.map(student => (
                                <tr key={student.id} className="border-t hover:bg-gray-50">
                                    ...
                                    <td className="py-3 px-4 text-text-secondary">{student.group?.name || 'N/A'}</td>
