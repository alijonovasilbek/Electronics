...
const Sidebar: React.FC<SidebarProps> = ({ currentPage, setCurrentPage, isOpen, setIsOpen, onLogout }) => {
  ...
  const navItems: { name: Page; icon: React.ReactElement }[] = [
    ...
    { name: 'Contracts', icon: <DocumentTextIcon /> },
    ...
    <>
      ...
      <aside className={`w-64 flex-shrink-0 bg-bunyodkor-blue text-white flex flex-col p-4 fixed inset-y-0 left-0 z-40 transform ${isOpen ? 'translate-x-0' : '-translate-x-full'} md:relative md:translate-x-0 transition-transform duration-300 ease-in-out`}>
        ...
        <div className="mt-auto">
             <button
                onClick={onLogout}
                className="flex items-center w-full px-4 py-3 text-sm font-medium text-white bg-red-500/20 rounded-lg hover:bg-red-500/40 transition-colors duration-200"
              >
                <LogoutIcon />
